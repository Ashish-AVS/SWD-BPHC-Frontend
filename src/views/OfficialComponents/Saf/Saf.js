    ...
    try{
      if (status == 0) {
        const fetchData0 = async () => {
          const result = await fetch(`${BaseUrl}/api/o/saf/get?status=0`, {
      ...
      if (status === 1) {
        const fetchData1 = async () => {
          const result = await fetch(`${BaseUrl}/api/o/saf/get?status=1`, {
            headers: { Authorization: `Bearer ${token}` }
  ...
  React.useEffect(()=>{
    ...
    try{
      const SendData=async ()=>{
        const result =await fetch(`${BaseUrl}/api/o/saf/export`,{
          headers:{Authorization:`Bearer ${token}`,
          Accept: "text/csv"}
        })
            ...
            tabs={[
              {
                ...
                tabContent: (
                  <MaterialTable
                  title="PENDING SAF APPLICATIONS"
                  columns={[
