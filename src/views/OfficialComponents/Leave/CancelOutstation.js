...
export default function CancelModal({ open, setOpen, cancelId, setIsUpdated }) {
    ...
    if (sendingData === true) {
      try {
        const sendData = async () => {
          setLoading(true);
          const result = await fetch(`${BaseUrl}/api/outstation/cancel`, {
            method: "post",
            headers: {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h4>
            Are you sure you want to cancel the your pending Outstation Request?
          </h4>
        </div>
      </DialogContent>
