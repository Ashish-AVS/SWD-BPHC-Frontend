  ...
  React.useEffect(() => {
    try {
      const fetchData = async () => {
        const result = await axios.get(`${BaseUrl}/api/notices/feed?start=0&limit=10`, {
          headers: {
            Authorization: token
          }
        })
        // const res = await result.json();
        if (result.data.err === false) { setNoticeData(result.data.data) } else {
                          ...
                          <GridItem xs={12} sm={4} md={4}>
                            <Card style={{ border: '1px solid black' }}>
                              ...
                              <CardFooter style={{ display: 'flex', justifyContent: 'center' }}>
                                <a target='_blank' href={data.attachment} style={{ textDecoration: 'none', color: 'white' }} rel='noreferrer'>
